...
они добавлены в репозиторий сабмодулями — [benchmarks](https://github.com/lsils/benchmarks.git), [iwls2024-ls-contest](https://github.com/alanminko/iwls2024-ls-contest.git), 
[hdl-benchmarks](./hdl-benchmarks/iccad-2015). Также для тестирования на произвольных формулах использовался набор данных
с [SAT competition 2021](tests/sat2021). Формулы слишком большие, загружать [скриптом](tests/sat2021/download.sh)
...
[модуль тестов](tests/scripts/create_tests.py). Поддерживаются два формата для схем — `.aig` (And-Inverter Graph) и `.v` (Verilog).
Используется специальная [утилита](https://github.com/Lipen/aigerox). Она доступна в Test PyPI.

Таким образом, схема из бинарного формата `.aig` или формата для схем `.v` конвертируется в ASCII-формат `.aag` 
с помощью утилиты [aiger](https://github.com/arminbiere/aiger),
...
Для схем также сразу генерируются экземпляры задачи LEC (Logical Equivalence Checking) — два экземляра одной схемы соединяются специальной 
структурой для проверки их на эквивалентность. Это необходимо для последнего этапа работы — применения метода нахождения входов для 
декомпозиции задачи LEC.

После этого тесты готовы, можно запускать решения и проверки. Для запуска решений используются [скрипты](scripts).
...
В [fast_orchestra](extraction/orchestra/fast_orchestra.py) есть единственный второй аргумент: `--fasten`, который ухудшает ответ, но ускоряет решение.
...
```
Пример запуска [extractor](extraction/extractor.py):
```bash
python3 extractor.py tests/cnf/adder.cnf --non-schemas
```

Затем ответы, найденные решениями, можно оценить и автоматически составить отчёт, этим занимается [модуль отчётов](util/report.py).
Отчет сохраняется в [reports](./reports), чтобы не засорять репозиторий, папка игнорируется гитом.

Для сбора статистики по конфликтам в подстановках для различных схем используется [скрипт](scripts/run-hypothesis-large-sets.sh).
В папку [stats](./stats) записываются наблюдения о конфликтах на различных размерах множеств для разных схем.
На этих наблюдениях затем можно построить графики с помощью [графического модуля](graphics/conflicts_graphics.py).

## Решения

### Переборные решения

- [fullscan_border](extraction/fullscan/fullscan_border.py): полный перебор всех возможных подмножеств
...
- [fullscan_3n](extraction/fullscan/fullscan_3n.py): полный перебор за $O(3^n)$. Минус по сравнению с [fullscan_border](extraction/fullscan/fullscan_border.py) в том, что нет
...
- [fast_orchestra](extraction/orchestra/fast_orchestra.py): ускоренная комбинация [greedy_expansion](extraction/heuristic/greedy_expansion.py), 
  [evolution](extraction/heuristic/evolution.py) и [cut_conflicts](extraction/heuristic/cut_conflicts.py), сначала расширяемся в сторону минимальных конфликтов, затем мутируем 
  получившееся множество, затем исключаем конфликтующие переменные.
- [extractor](extraction/extractor.py): модуль для внешнего использования, меняет конфигурации поиска входов в 
  зависимости от размера и структуры схем — для большой схемы запускает упрощённый поиск, который работает быстрее, но 
  находит ответ хуже и наоборот.
