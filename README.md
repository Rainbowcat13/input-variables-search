...
У [evolution](./evolution.py) и [orchestra](./orchestra.py) есть хелперы для аргументов командной строки, можно посмотреть, что они принимают. 
В [fast_orchestra](./fast_orchestra.py) есть единственный второй аргумент: `--fasten`, который (предположительно) ухудшает ответ, но ускоряет решение.
Пример запуска [evolution](./evolution.py):
```bash
...
На этих наблюдениях затем можно построить графики с помощью [графического модуля](./conflicts_graphics.py).
